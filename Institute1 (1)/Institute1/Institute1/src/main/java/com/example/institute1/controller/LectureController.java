...
import com.example.institute1.BO.LectureBOImpl;
...
public class LectureController implements Initializable {
    LectureBOImpl lectureBO = (LectureBOImpl) BOFactory.getBoFactory().getBO(BOFactory.BOTypes.LECTUREBO);
    @FXML
    ...
    void lectureSave(ActionEvent event) {
        try {
            if (validateLectureInputs()) {
                ...
                if (lectureBO.saveLecture(lectureDto)) {
                    new Alert(Alert.AlertType.INFORMATION, "Lecture Saved").show();
                    refreshTable();
                    lblLectureId.setText(lectureBO.getNextLectureID());
                    lectureReset(null);
                } else {
    ...
    void lectureUpdate(ActionEvent event) {
        ...
        if (selectedLecture != null) {
            ...
            if (buttonType.isPresent() && buttonType.get() == ButtonType.YES) {
                ...
                try {
                    if (lectureBO.updateLecture(updatedLectureDto)) {
    ...
    void lectureDelete(ActionEvent event) {
        ...
        if (selectedLecture != null) {
            ...
            if (buttonType.isPresent() && buttonType.get() == ButtonType.YES) {
                try {
                    if (lectureBO.deleteLecture(selectedLecture.getLectureId())) {
    ...
    private void refreshTable() {
        try {
            List<LectureDto> lectures = lectureBO.getAllLectures();
