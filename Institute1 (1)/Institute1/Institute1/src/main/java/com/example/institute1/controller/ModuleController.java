...
import com.example.institute1.BO.ModuleBOImpl;
...
public class ModuleController implements Initializable {
    ModuleBOImpl moduleBO = (ModuleBOImpl) BOFactory.getBoFactory().getBO(BOFactory.BOTypes.MODULEBO);
    ...
    void moduleSave(ActionEvent event) {
        try {
            if (validateModuleInputs()) {
                ...
                if (moduleBO.saveModule(moduleDto)) {
                    new Alert(Alert.AlertType.INFORMATION, "Module Saved").show();
                    refreshTable();
                    lblModuleId.setText(moduleBO.getNextModuleID());
                    ModuleReset(null);
                } else {
    ...
    void moduleUpdate(ActionEvent event) {
        ...
        if (selectedModule != null) {
            ...
            if (buttonType.isPresent() && buttonType.get() == ButtonType.YES) {
                ...
                try {
                    if (moduleBO.updateModule(updatedModuleDto)) {
    ...
    void moduleDelete(ActionEvent event) {
        ...
        if (selectedModule != null) {
            ...
            if (buttonType.isPresent() && buttonType.get() == ButtonType.YES) {
                try {
                    if (moduleBO.deleteModule(selectedModule.getModuleId())) {
    ...
    private void refreshTable() {
        try {
            List<ModuleDto> modules = moduleBO.getAllModules();
