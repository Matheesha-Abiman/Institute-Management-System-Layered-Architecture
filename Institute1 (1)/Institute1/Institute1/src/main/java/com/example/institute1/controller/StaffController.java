...
import com.example.institute1.BO.StaffBOImpl;
...
public class StaffController implements Initializable {
    StaffBOImpl staffBO = (StaffBOImpl) BOFactory.getBoFactory().getBO(BOFactory.BOTypes.STAFFBO);
    ...
    void staffSave(ActionEvent event) {
        try {
            if (validateStaffInputs()) {
                ...
                if (staffBO.saveStaff(staffDto)) {
                    new Alert(Alert.AlertType.INFORMATION, "Staff saved successfully").show();
                    refreshTable();
                    lblStaffId.setText(staffBO.getNextStaffID());
                    staffReset(null);
                } else {
        ...
        if (selectedStaff != null) {
            ...
            if (buttonType.get() == ButtonType.YES) {
                try {
                    ...
                    if (staffBO.updateStaff(updatedStaffDto)) {
    ...
    void staffDelete(ActionEvent event) {
        ...
        if (selectedStaff != null) {
            ...
            if (buttonType.get() == ButtonType.YES) {
                try {
                    if (staffBO.deleteStaff(selectedStaff.getStaffId())) {
    ...
    private void refreshTable() {
        try {
            List<StaffDto> staffs = staffBO.getAllStaff();
